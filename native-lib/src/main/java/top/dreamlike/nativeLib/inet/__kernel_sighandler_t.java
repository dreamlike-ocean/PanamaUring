...
public interface __kernel_sighandler_t {
    ...
    static MemorySegment allocate(__kernel_sighandler_t fi, MemorySession session) {
        return RuntimeHelper.upcallStub(__kernel_sighandler_t.class, fi, constants$1.__kernel_sighandler_t$FUNC, session);
    }
    ...
    static __kernel_sighandler_t ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int __x0) -> {
            try {
                constants$1.__kernel_sighandler_t$MH.invokeExact((Addressable)symbol, __x0);
            } catch (Throwable ex$) {
