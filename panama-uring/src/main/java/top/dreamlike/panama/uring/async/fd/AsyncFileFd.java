...
public record AsyncFileFd(IoUringEventLoop ioUringEventLoop, int fd) implements IoUringAsyncFd {
    ...
    public static CancelableFuture<AsyncFileFd> asyncOpen(IoUringEventLoop ioUringEventLoop, OwnershipMemory path, int flags, int mode) {
        return (CancelableFuture<AsyncFileFd>) ioUringEventLoop.asyncOperation(sqe -> Instance.LIB_URING.io_uring_prep_openat(sqe, -1, path.resource(), flags, mode))
