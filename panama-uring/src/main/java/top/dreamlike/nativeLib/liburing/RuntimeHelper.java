...
final class RuntimeHelper {
    ...
    static <Z> MemorySegment upcallStub(Class<Z> fi, Z z, FunctionDescriptor fdesc, Arena session) {
        try {
            MethodHandle handle = MH_LOOKUP.findVirtual(fi, "apply", fdesc.toMethodType());
            handle = handle.bindTo(z);
            return LINKER.upcallStub(handle, fdesc, session.scope());
        } catch (Throwable ex) {
    ...
    static MemorySegment asArray(MemorySegment addr, MemoryLayout layout, int numElements, Arena session) {
        return MemorySegment.ofAddress(addr.address(), numElements * layout.byteSize(), session.scope());
    }
